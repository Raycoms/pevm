...
contract Benchmark {
    ...
    uint256 public constant NO_PERMS = 7;

    constructor() {
        // Init module: populate with (i,1) for i=0..99
        for (uint256 i = 0; i < 52000; i++) {
            resourceTable[i] = 1;
        }
    }
    ...
    function loopExchange(uint256 loopCount, uint256[] calldata resources) external {
        ...
        for (uint256 i = 0; i < length; i++) {
            uint256 res = resources[i];
            resourceTable[res] = resourceTable[res] + 1;
        }

        // Second loop: loopCount iterations, cycling through resources
        for (uint256 i = 0; i < loopCount; i++) {
            uint256 j = i % length;
            uint256 res = resources[j];
            resourceTable[res] = resourceTable[res] + 1;
        }
    }
    ...
    function exchange(uint256 resource) external {
        for (uint256 i = 0; i < 8; i++) {
            resourceTable[resource] = resourceTable[resource] + 1;
        }
    }
    ...
    function exchangeTwo(uint256 resource1, uint256 resource2) external {
        for (uint256 i = 0; i < 8; i++) {
            resourceTable[resource1] = resourceTable[resource1] + 1;
            resourceTable[resource2] = resourceTable[resource2] + 1;
        }
    }
}
